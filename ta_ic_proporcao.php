    ...
    if(isset($_POST['calcular'])){
        if (!empty($_POST['r_confianca'])) {
            if (is_numeric($_POST['t_erro'])) { 
                if (is_numeric($_POST['t_proporcao'])) {
                    $z = get_z($_POST['r_confianca']);
                    $erro = $_POST['t_erro'];
                    $p = $_POST['t_proporcao'];

                    if($erro < 0.001 || $erro > 100) echo "<h3 class='erro'>Valor inválido para o Erro.</h3>";
                    else if($p < 0.001 || $p > 100) echo "<h3 class='erro'>Valor inválido para a Proporção Estimada na População.</h3>";
                    else {
                        $erro = $erro/100;
                        $p = $p/100;

                        $n = $z*$z*$p*(1-$p)/($erro*$erro);
                        $n_anterior = $n;
                ...
                else echo "<h3 class='erro'>Valor inválido para a Proporção Estimada na População.</h3>";
            }
...
<body>
    ...
    <div id="content">
        ...
        <h3 class="titulo">Intervalo de Confiança de uma Proporção</h3>
        ...
        <form method="POST">
            ...
            <?php $p = (!empty($_POST['t_proporcao']) ? $_POST['t_proporcao'] : ''); ?>
            <label for="t_proporcao" title="A proporção que você espera que ocorra na população. Caso não tenha ideia do que espera encontrar entre com 50%.">Proporção Estimada na População (%):</label>
            <input type="number" id="t_proporcao" name="t_proporcao" min="0" max="100" step="any" value="<?php echo $p; ?>"><br />
